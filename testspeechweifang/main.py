...
def main():
    ...
    if args.voice:
        ...
        translation_continuous(file_path)
...
def save_api_key() -> str:
    while 1:
        ...
        region = input("請輸入Azure 語音服務 region: ")
        if api_key:
            break
    with open('settings.json', 'w') as fp:
        json.dump({'api': api_key,'region': region}, fp)
...
area=load_region()
speech_config = speechsdk.SpeechConfig(subscription=API_KEY, region=area,
